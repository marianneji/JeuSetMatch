...
class ViewController: UIViewController {
    ...
    // MARK: - Outlets
    @IBOutlet var p1ScoreLabels: [UILabel]!
    @IBOutlet var p2ScoreLabels: [UILabel]!
    lazy var pScoreLabels: [Player: [UILabel]] = [Player.one: p1ScoreLabels, Player.two: p2ScoreLabels]

    @IBOutlet weak var p1GameScoreLabel: UILabel!
    @IBOutlet weak var p2GameScoreLabel: UILabel!
    lazy var playerGameScoreLabels: [Player: UILabel] = [Player.one: p1GameScoreLabel, Player.two: p2GameScoreLabel]
    ...
    private func updateAllSetsScore(forPlayer player: Player) {
        for index in 0..<5 {
            if index < match.sets.count {
                ...
                pScoreLabels[player]![index].text = "\(set.scores[player]!)"
            } else {
                pScoreLabels[player]![index].text = ""
            }
        }
    }
